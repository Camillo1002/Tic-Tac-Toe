...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        int blankSpace = 0;
        //ten fragment kodu zlicza liczbę pustych miejsc oznaczonych jako '-'
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (grid[i][j] == '-') {
                    blankSpace++;
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                //Ten warunek pomijaj puste miejsca na planszy gry
                if (grid[i][j] != '-') {
                    if (j == 0) {
                  ...
                  /*
                    Ten warunek sprawdza, czy wszystkie komórki na planszy są już wypełnione i Jeśli oba te warunki są prawdziwe, to ogłasza, że wynikiem jest remis ("tie")
                   */

                    if (blankSpace == 0 && result.equals("None")) {
                        result = "tie";
                    }
                }
            }
